                              ...
                              alphabetical.order=FALSE,boxplot.type="ggplot",balance.classes=FALSE,deeplearning=FALSE,alpha.col=1,ggplot.type1=NA)
{
  ...
  if(split.train.test==TRUE){
    if(nsamp<30){
      ...
      if(all(is.na(confounder.matrix))==FALSE){
  ...
  if(all(is.na(good_feats_index))==TRUE){

    if(all(is.na(feat.sel.methods))==FALSE){
    ...
    #if(all(is.na(Xtest))==FALSE)
    {
      ...
      eval_mat_perm<-lapply(1:nperm,function(p1)
      {
        ...
        if(all(is.na(class_scda))==FALSE){
          ...
          if(all(is.na(testeval_res3))==FALSE){
      ...
      if(all(is.na(Xtest))==FALSE){
        ...
        if(length(good_feats_index)>2)
        {
          ...
          if(deeplearning==TRUE){
            ...
            if(all(is.na(test_data_check))==TRUE){
